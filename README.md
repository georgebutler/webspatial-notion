...
The Solar System document uses one combined GLB asset at `/glb/Planets.glb`. The asset contains the Sun and all eight planets plus one authored looping `Orbit` action on the eight planet parent empties; the Sun is not animated. The overview uses the model element's native `autoPlay` and `loop` attributes plus a load-time `play()` trigger, while the individual planet previews use the existing model-ref rotation. The GLB export embeds the planet texture images so the model does not depend on Blender's source-file paths. Native `<model>` support is currently limited, so regular browsers load the Immersive Web model-element polyfill from `public/model-element-polyfill.js` during startup.
...
If you change the model markup, keep the `<source type="model/gltf-binary">` child and a useful fallback image. The polyfill is loaded automatically by `src/main.tsx` only when the browser does not already expose `HTMLModelElement`; WebSpatial environments continue using the SDK's native `<Model>` integration.
